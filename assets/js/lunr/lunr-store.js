var store = [{
        ...
        "title": "柯洁能成为九冠王吗？",
        "excerpt":"柯洁（九段）的上一个世界冠军可以追溯到 2020 年三星杯 2-0 胜申真谞（九段），连续两年没有获得世界冠军，上一次和世界冠军失之交臂是 2023 年亚运会负許皓鋐（九段）。不由让人疑问，柯洁在职业生涯再拿一个世界冠军的概率有多少？为了回答这个问题，我们需要一个衡量棋手水平的模型。一个比较简单的方法就是通过 Elo 等级分。 Bradley-Terry 模型 今天的 Elo 等级分和 Arpad Elo 的原始假设有比较大的差异，尤其是在选择的分布上。一般我们会假设使用 Bradley-Terry 模型构造 elo 等级分模型，即对于两个选手 A 和 B，A 获胜的概率是 \\(P(A&gt;B)=\\frac{10^\\frac{R(A)}{400}}{10^\\frac{R(A)}{400}+10^\\frac{R(B)}{400}}\\) 然而当两位选手下完一盘棋后，后验的结果如何反馈到 elo 分的修正上是一个比较大的问题。一个比较好的算法是 WHR Algorithm，也就是 Go Ratings 使用的算法。 举个例子来说，我们以截止到 2024 年 5 月 10 日数据的最新等级分来看的话，如果我们比较柯洁和战鹰（二段）的等级分的话，柯洁的等级分是 3694 分，而战鹰的等级分是 2824 分。如果明天就有一盘柯洁和申真谞的比赛的话，那么战鹰战胜柯洁的概率是 0.66%，从中可以看出顶尖职业棋手间微妙的差距。 申真谞的等级分是 3877 分，所以柯洁对战申真谞胜出的概率是 25.86%。而柯洁前两天爆冷负刘宇航（六段），刘宇航的等级分是...","categories": [],
        "tags": ["统计学","Elo"],
